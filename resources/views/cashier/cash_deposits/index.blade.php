...
<!-- Stats Section -->
<div class="mb-6">
    <h2 class="text-xl font-bold mb-4">Your Deposit Statistics</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <!-- All-Time Deposits -->
        <div class="bg-white p-4 rounded-lg shadow">
            <h4 class="text-gray-500 text-sm font-medium">YOUR ALL-TIME DEPOSITS</h4>
...
<div class="space-y-8">
    <!-- Toggleable Deposit Form -->
    <div x-data="{ open: false }">
        <button @click="open = !open" class="flex items-center justify-between w-full px-6 py-4 text-lg font-semibold text-left text-white bg-indigo-500 rounded-lg hover:bg-indigo-600 focus:outline-none">
        ...
        <div x-show="open" x-transition class="mt-4 bg-white p-6 rounded-lg shadow-md">
            <form action="{{ route('cashier.cash-deposits.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div>
                        ...
                        <select id="wallet_id" name="wallet_id" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                            @foreach(App\Models\Wallet::with('currency')->get() as $wallet)
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <h3 class="text-lg font-semibold mb-4">Your Deposit History</h3>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-4 text-center text-gray-500">No deposits found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (viewButtons.length && modalBody) {
            viewButtons.forEach(button => {
                button.addEventListener('click', () => {
                    const deposit = JSON.parse(button.dataset.deposit);

                    let userName = deposit.user ? deposit.user.name : 'N/A';
